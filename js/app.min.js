...
const initMarquee = () => {
  ...
  items.forEach((wrapper) => {
    t.el = wrapper.querySelector(".marquee__toright");
    if (!t.el) return;

    t.width = t.el.offsetWidth;
    t.el.innerHTML += t.el.innerHTML;

    // Antes: marqueeRight(t.el, 30, "+=50%")
    // Ahora: 45 → un poco más lento pero no exagerado
    let timeline = gsap.timeline().add(marqueeRight(t.el, 45, "+=50%"), 0);

    // Antes: timeScale base 1, ahora 0.8 (apenas más tranquilo)
    let resetTween = gsap.to(timeline, {
      duration: 1.2,
      timeScale: 0.8,
      paused: true,
    });

    // Antes: clamp(1, 6) → podía volverse loquísimo
    // Ahora: rango intermedio: no tan loco, pero sigue reaccionando
    const clampSpeed = gsap.utils.clamp(0.6, 2.5);

    ScrollTrigger.create({
      start: 0,
      end: "max",
      onUpdate: (st) => {
        // Antes: /200 → súper sensible
        // Ahora: /350 → reacciona pero más suave
        const velocity = Math.abs(st.getVelocity() / 350);

        timeline.timeScale(clampSpeed(velocity || 0.8));
        resetTween.invalidate().restart();
      },
    });
  });
};

const marqueeRight = (el, duration, delta) => {
  ...
  return gsap.to(el, {
    ...
    modifiers: {
      x(value) {
        return (delta = wrap(parseFloat(value)) + "%");
      },
    },
    repeat: -1,
  });
...
const themeBtn = document.querySelector("#scroll-to-top");
